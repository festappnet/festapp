...
DELETE_ACCOUNT_SUPABASE_URL="$SUPABASE_URL"
DELETE_ACCOUNT_ANON_KEY="$SUPABASE_ANON_KEY"
if [ "$BACKEND_ACTIVATION_PHASE" = canonical ]; then
    DELETE_ACCOUNT_SUPABASE_URL="$BACKEND_ACTIVATION_CANONICAL_SUPABASE_URL"
    DELETE_ACCOUNT_ANON_KEY="$BACKEND_ACTIVATION_CANONICAL_SUPABASE_ANON_KEY"
fi
python3 - "$DELETE_ACCOUNT_TEMPLATE" "$DELETE_ACCOUNT_FILE" "$APP_NAME" \
    "$DELETE_ACCOUNT_SUPABASE_URL" "$DELETE_ACCOUNT_ANON_KEY" <<'PY'
import html
import json
import sys
