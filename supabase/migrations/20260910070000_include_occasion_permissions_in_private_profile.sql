...
GRANT EXECUTE ON FUNCTION public.get_private_profile_payload_v1(bigint,uuid)
  TO service_role;

UPDATE public.client_sync_component_sources
SET tracked_columns = ARRAY(
  SELECT DISTINCT column_name
  FROM unnest(tracked_columns || ARRAY[
    'is_editor','is_editor_view','is_editor_order','is_editor_order_view',
    'is_approved','is_approver','is_manager'
  ]) AS column_name
...
UPDATE public.client_sync_private_scopes
