...
-- Preserve legacy map icons addressed by places.type as well as icons referenced
-- by numeric foreign keys. The body stays explicit so this forward migration is
...
DECLARE v_revision bigint; v_payload jsonb; v_organization bigint; v_unit bigint; v_generated_at timestamptz;
BEGIN
  ...
  CASE p_component
    ...
    WHEN 'map_catalog' THEN
      SELECT o.organization,o.unit INTO v_organization,v_unit FROM public.occasions o WHERE o.id=p_scope_id;
      WITH visible_places AS (
        ...
        SELECT icon FROM visible_paths WHERE icon IS NOT NULL UNION
        SELECT i.id FROM visible_places p JOIN public.icons i
          ON i.organization=v_organization AND (i.unit=v_unit OR i.unit IS NULL)
          AND lower(btrim(i.link))=lower(btrim(p.type))
...
WITH affected AS (
  SELECT DISTINCT p.occasion AS scope_id
  FROM public.places p
  JOIN public.occasions o ON o.id=p.occasion
  JOIN public.icons i ON i.organization=o.organization
    AND (i.unit=o.unit OR i.unit IS NULL)
    AND lower(btrim(i.link))=lower(btrim(p.type))
  WHERE NOT p.is_hidden AND p.type IS NOT NULL AND btrim(p.type)<>''
), bumped AS (
  UPDATE public.client_sync_scopes s
  SET source_revision=s.source_revision+1,updated_at=now()
  FROM affected a
  WHERE s.component='map_catalog' AND s.scope_type='occasion'
    AND s.scope_id=a.scope_id
  RETURNING s.component,s.scope_type,s.scope_id,s.source_revision
)
INSERT INTO public.client_projection_dirty_keys
  (component,scope_type,scope_id,entity_id,source_revision)
SELECT component,scope_type,scope_id,0,source_revision FROM bumped
